use crate::_tachys::view::{Position, RenderHtml};
use futures::Stream;
use std::{
...
impl StreamBuilder {
        ...
        View: RenderHtml,
    {
        ...
        fallback.to_html_with_buf(&mut self.sync_buf, position, true, mark_branches);
                        ...
                        match pending.as_mut().poll(cx) {
                            Poll::Ready(OooChunk {
                                ...
                                if let Some(start) = placeholder_at {
                                    ...
                                    let (_, after) = replaced.split_at(end - start + closing.len());
                                    ...
                                    OooChunk::push_end(replace, &id, &mut this.sync_buf);
                                }
                                self.poll_next(cx)
                            }
                            Poll::Pending => {
