use crate::tachys::{
    ...
    renderer::{types, Rndr},
    ssr::StreamBuilder,
    view::{add_attr::AddAnyAttr, Mountable, Position, Render, RenderHtml},
};
    ...
    V::State: 'static,
{
    ...
    {
        let value = self.invoke();
        value.to_html_async_with_buf::<OUT_OF_ORDER>(buf, position, escape, mark_branches);
    }
}
    ...
    V::State: 'static,
{
    ...
    fn hydrate<const FROM_SERVER: bool>(mut self, key: &str, el: &types::Element) -> Self::State {
    ...
    fn build(mut self, el: &types::Element, key: &str) -> Self::State {
        let key = Rndr::intern(key);
        let key = key.to_owned();
        let el = el.to_owned();
